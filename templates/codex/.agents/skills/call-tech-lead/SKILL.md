...
> Adapter Codex genere depuis le template Claude. Invoquer explicitement avec `$call-tech-lead` ou laisser Codex le choisir par sa description.
...
Protocole strict :
1. Si un signal commercial fort est détecté en phase 1 (feature pricing / différenciateur / refonte plan / nouveau segment), tech-lead **propose** à l'utilisateur via `question utilisateur` :
...
3. Crée `.codex/runs/call-tech-lead/<YYYYMMDD-HHMMSS>-<slug>/` (inclus dans `.gitignore`).
...
- **Mode semi** : `question utilisateur` à l'utilisateur — "Signal commercial `<X>` détecté. Consulter `<agents>` (+~100k tokens) ? Oui / Non". Default = Non.
...
Invoque l'agent `po-metier` via le mecanisme de subagents Codex avec le besoin initial. L'agent produit une US structurée (sauvée dans `docs/us/US-<slug>.md` + copie dans `02-us.md`).

**Checkpoint `--mode=semi`** :
- Affiche l'US dans le chat.
- Pose la question : "Valide l'US ? (oui / modifier / stop)"
- Si "modifier" → `question utilisateur` pour collecter les ajustements, re-délègue au PO.
...
Pour **chaque agent convoqué sauf PO** (déjà intervenu), tech-lead délègue en parallèle (plusieurs subagent spawning calls dans le même message) :
...
**Règle d'escalade** :
- **Mode `--mode=semi`** : si désaccord persiste après 2 rounds sur un point Critique → `question utilisateur` à l'utilisateur.
...
**Mis à jour incrémentalement à chaque phase** (pas écrit en une fois à la fin). L'utilisateur peut faire `tail -f .codex/runs/call-tech-lead/<slug>/TRANSCRIPT.md` en direct.
...
## Répartition modèles (OpenAI/Codex)

Pour préserver le budget du modèle premium, seuls les agents dont le raisonnement est **vraiment complexe** utilisent le modele Codex adapte a leur role :

| Agent | Modèle | Raison |
|---|---|---|
| `full-stack-lead` | **gpt-5.3-codex / high** | Raisonnement architectural complexe, choix de patterns |
| `cso` | **gpt-5.3-codex / high** | Threat modeling + STRIDE = raisonnement multi-facteurs |
| `po-metier` | gpt-5.4-mini | Rédaction US structurée, pattern matching domaine |
{{#IF HAS_DESIGN_SYSTEM}}
| `designer-uxui` | gpt-5.4-mini | Application du design system, rédaction copy |
{{/IF}}
| `data-engineer` | gpt-5.3-codex / medium | Requêtes SQL, index et migrations — raisonnement code/data |
| `qa` | gpt-5.3-codex / medium | Exécution tests, cartographie US→tests |
{{#IF HAS_AI_FEATURE}}
| `ai-llm-engineer` | gpt-5.3-codex / medium | Prompts / parsers / validators IA — domaine bien cadré (pattern-matching) |
{{/IF}}
...
1. `full-stack-lead` (gpt-5.3-codex) — ~35%
2. `qa` (gpt-5.3-codex) — ~20%
3. `cso` (gpt-5.3-codex, si convoqué) — ~15%
4. `po-metier` (gpt-5.4-mini) — ~10%
5. Autres agents gpt-5.4-mini — ~10% combiné
6. Orchestrateur — ~10%
...
- Répartition par agent :
  - full-stack-lead (gpt-5.3-codex) : ~Xk (~$Y)
  - qa (gpt-5.3-codex) : ~Xk (~$Y)
  - ...
- Durée totale : <MmSs>
- Budget plan : <impact estimé selon fournisseur>
```
...
- Agents : `.codex/agents/full-stack-lead.toml`, `po-metier.toml`{{#IF HAS_DESIGN_SYSTEM}}, `designer-uxui.toml`{{/IF}}, `qa.toml`, `cso.toml`, `data-engineer.toml`{{#IF HAS_AI_FEATURE}}, `ai-llm-engineer.toml`{{/IF}}
- Skills utilisés : `.agents/skills/{redige-us, lead-tech, review-pr, qa-flow, ship-pr, security-audit}/SKILL.md`
