...
> Adapter Codex genere depuis le template Claude. Invoquer explicitement avec `$setup-project` ou laisser Codex le choisir par sa description.
> Les artefacts de run Codex sont attendus sous `.codex/runs/<skill>/<timestamp-slug>/`.
# $setup-project

## Objectif
Configurer le starter kit d'équipes virtuelles Codex en une seule session, via un questionnaire structuré. À la fin, tous les placeholders `{{VARIABLE}}` dans `.codex/agents/`, `.agents/skills/` et `docs/` sont remplacés par les valeurs du projet.
...
- `.codex/agents/*.toml` (13 agents avec placeholders)
- `.agents/skills/*/SKILL.md` (16 skills avec placeholders)
- `.agents/skills/*/SKILL.md` (16 alias)
...
Vérifier avec `ls .codex/agents/ | head -3` avant de lancer.

## Garde-fous

- ❌ JAMAIS modifier des fichiers hors de `.agents/`, `.codex/`, `docs/` ou `backlog.md`.
...
- ✅ Backup automatique avant modifications : le skill crée `.codex.backup-<timestamp>/` avant tout remplacement.
...
ls -la .codex/agents/ .agents/skills/ docs/GUIDE-LLM.md backlog.md

# Compter les placeholders résiduels
grep -r "{{" .agents/ .codex/ docs/ | wc -l
```
...
2. **Outil principal** : Codex / OpenAI Codex / les deux.
...
17. **LLM dans le produit** : OpenAI OpenAI/Codex / OpenAI / Google Gemini / Mistral / autre / none (active/désactive `ai-llm-engineer`)
...
Pour chaque fichier avec placeholders (`.codex/agents/*.toml`, `.agents/skills/*/SKILL.md`, `docs/*.md`) :
...
```bash
rm .codex/agents/<nom>.toml
```

Et supprimer les références à cet agent dans :
- `.agents/skills/call-tech-lead/SKILL.md` (matrice de routing)
- `.agents/skills/call-growth-lead/SKILL.md` (matrice de routing)
- `docs/EQUIPES-LLM.md`
...
grep -rn "{{" .agents/ .codex/ docs/ | grep -v ".template.md" | grep -v "{{#IF" | grep -v "{{/IF}}"
```
...
```bash
rm -rf .codex.backup-* docs.backup-* backlog.backup-*
```
```
...
```bash
git add .agents/ .codex/ docs/ backlog.md
...
- Les backups restent disponibles pour rollback manuel : `rm -rf .codex && mv .codex.backup-<ts> .codex`.
...
- Signaler l'erreur et donner la commande exacte :
  ...
  cp -r templates/codex <projet>/.codex
...
- ❌ Modifier `.codex/config.toml` — fichier hors scope du skill.
