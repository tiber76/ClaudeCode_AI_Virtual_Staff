...
> Adapter Codex genere depuis le template Claude. Invoquer explicitement avec `$retro-campagne` ou laisser Codex le choisir par sa description.
...
- Ajout dans `.codex/agents/<agent>.toml` section "Anti-patterns" ou "Style"
...
- [ ] `.codex/agents/<agent>.toml` → ajout anti-pattern : <texte>
- [ ] `.codex/agents/<agent>.toml` → ajout référence : <texte>
...
- `.agents/skills/call-growth-lead/SKILL.md` — priorisation next bets
- `.codex/agents/marketing-analytics.toml` — mesure écarts
- `.agents/skills/retro/SKILL.md` — retro côté dev (même esprit)
