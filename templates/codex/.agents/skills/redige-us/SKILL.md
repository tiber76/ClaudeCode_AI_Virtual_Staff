...
> Adapter Codex genere depuis le template Claude. Invoquer explicitement avec `$redige-us` ou laisser Codex le choisir par sa description.
...
2. **Max 3 questions de clarification** via `question utilisateur`, uniquement si vraiment bloquant. Préférer noter dans "Questions ouvertes" quand c'est un détail.
...
Si un élément critique manque, poser max 3 questions via `question utilisateur`. Exemples :
